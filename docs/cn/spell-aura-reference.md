...
[`返回至:法术效果参考`](spell-effects-reference)

[`返回至:spell_dbc`](spell_dbc)


### 本页包含关于 EffectMiscValue 及其他法术光环（Spell Aura）值的用法说明。
### 仅包含已知会使用 EffectMiscValue 或其他非惯例值的法术光环。
...
- BasePoints：（+Die Points）可以被魅惑的等级上限
...
- EffectMiscValueA：潜行类型（0 = 普通，1 = 陷阱）
...
- EffectMiscValueA：来自 SpellShapeshiftForm.dbc 的 ID

SPELL_AURA_EFFECT_IMMUNITY = 37
- EffectMiscValueA：要对其免疫的 [Spell Effect](spell-effects-reference) 的 ID。

SPELL_AURA_STATE_IMMUNITY = 38
- EffectMiscValueA：来自法术光环的 ID
...
- EffectMiscValueA：评级类型（Rating Type）
...
- EffectMiscValueA：要转化的符文类型
- EffectMiscValueB：转化后的新符文

SPELL_AURA_MOD_MECHANIC_DAMAGE_TAKEN_PERCENT = 255
- EffectMiscValueA：[Spell Mechanic](#spell-mechanic)

SPELL_AURA_NO_REAGENT_USE = 256
- EffectMiscValueA：某种材料类型。

SPELL_AURA_MOD_HOT_PCT = 259
- EffectMiscValueA：[School Mask](#school-mask)

SPELL_AURA_SCREEN_EFFECT = 260
- EffectMiscValueA：来自 ScreenEffect.dbc 的 ID

SPELL_AURA_PHASE = 261
- EffectMiscValueA：阶段掩码（Phase Mask）
...
- EffectMiscValueA：来自 OverrideSpellData.dbc 的 ID

SPELL_AURA_PREVENT_REGENERATE_POWER = 294
- EffectMiscValueA：[Power Type](#power-type)

SPELL_AURA_SET_VEHICLE_ID = 296
- EffectMiscValueA：来自 Vehicle.dbc 的 ID
...
- EffectMiscValueA：来自 AuraStateType 的 ID（master/src/server/shared/SharedDefines.h）
