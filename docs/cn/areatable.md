...
此 DBC 包含区域（zone）和子区域（subzone）列表。出于本文目的，将包含区域 ID、名称、mapID 和 AreaID。
...
| 1      | ID               | Int    | 区域 Area                                                      |
| 2      | MapID            | Int    | 地图或大陆                                                     |
| 3      | AreaID           | Int    | 地图的子区域 SubArea                                           |
| 4      | Explore Flag     | Int    |                                                               |
| 5      | Flags            | Int    |                                                               |
| 8      | SoundAmbienceID  | Int    | 引用白天和夜晚使用哪种环境音效                                  |
| 9      | ZoneMusicID      | Int    | 引用播放哪个区域音乐                                            |
| 10     | ZoneIntroMusicID | Int    | 引用进入区域时播放哪个区域开场音乐                              |
| 11     | Area Level       | Int    |                                                               |
| 12-28  | Area Name        | String |                                                               |
| 29     | FactionGroupID   | Int    | 拥有该区域的阵营                                                |
...
| Value      | Name                     | Notes                                                                                                                                                                                          |
| ---------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0x00000001 | AREA_FLAG_SNOW           | 雪地（仅丹莫罗、纳克萨玛斯、剃刀沼泽和冬泉谷）                                                                                                                          |
| 0x00000002 | AREA_FLAG_UNK1           | 可能是墓地？                                                                                                                                                                             |
| 0x00000004 | AREA_FLAG_UNK2           | 仅用于地图 571 上的区域（之前的开发）                                                                                                                                            |
| 0x00000008 | AREA_FLAG_SLAVE_CAPITAL  | 城市和城市子区域                                                                                                                                                                         |
| 0x00000010 | AREA_FLAG_UNK3           | 找不到常见含义                                                                                                                                                                      |
| 0x00000020 | AREA_FLAG_SLAVE_CAPITAL2 | 从属主城标志？                                                                                                                                                                       |
| 0x00000040 | AREA_FLAG_ALLOW_DUELS    | 允许在此决斗                                                                                                                                                                             |
| 0x00000080 | AREA_FLAG_ARENA          | 竞技场，包括副本竞技场和世界竞技场                                                                                                                                                         |
| 0x00000100 | AREA_FLAG_CAPITAL        | 主要主城标志                                                                                                                                                                         |
| 0x00000200 | AREA_FLAG_CITY           | 仅用于一个名为"City"的区域（它位于哪里？）                                                                                                                                             |
| 0x00000400 | AREA_FLAG_OUTLAND        | 资料片区域？（只有风暴之眼没有此标志，但它有 0x00004000 标志）                                                                                                          |
| 0x00000800 | AREA_FLAG_SANCTUARY      | 庇护所区域（PvP 禁用）                                                                                                                                                                  |
| 0x00001000 | AREA_FLAG_NEED_FLY       | 仅灵翼悬岛、萨瑟里斯王座、风暴要塞、禁魔监狱、生态船、能源舰、悲伤之翼点、龙颅脊、灵翼矿洞、龙喉营地、龙喉空中航线 |
| 0x00002000 | AREA_FLAG_UNUSED1        | 现在未使用（3.0.3 中没有任何区域/地区设置此标志）                                                                                                                                       |
| 0x00004000 | AREA_FLAG_OUTLAND2       | 资料片区域？（只有鲜血之环竞技场没有此标志，但它有 0x00000400 标志）                                                                                                     |
| 0x00008000 | AREA_FLAG_OUTDOOR_PVP    | PvP 目标区域？（死亡之门也有此标志，尽管它不是 PvP 目标区域）                                                                                                         |
| 0x00010000 | AREA_FLAG_ARENA_INSTANCE | 仅由副本竞技场使用                                                                                                                                                                  |
| 0x00020000 | AREA_FLAG_UNUSED2        | 现在未使用（3.0.3 中没有任何区域/地区设置此标志）                                                                                                                                       |
| 0x00040000 | AREA_FLAG_CONTESTED_AREA | 在 PvP 服务器上这些区域被视为争夺区域，即使其所在地区属于部落/联盟领土。                                                                    |
| 0x00080000 | AREA_FLAG_UNK6           | 瓦加德和魔枢：黑锋要塞                                                                                                                                                            |
| 0x00100000 | AREA_FLAG_LOWLEVEL       | 用于一些 area_level <=15 的起始区域                                                                                                                                              |
| 0x00200000 | AREA_FLAG_TOWN           | 有旅店的小镇                                                                                                                                                                           |
| 0x00400000 | AREA_FLAG_UNK7           | 战歌要塞、魔枢：黑锋要塞、新阿加曼德旅店、复仇之潮旅店                                                                                                                   |
| 0x00800000 | AREA_FLAG_UNK8           | 西部卫戍旅店、魔枢：黑锋要塞、瓦加德                                                                                                                                                |
| 0x01000000 | AREA_FLAG_WINTERGRASP    | 冬拥湖及其子区域                                                                                                                                                                  |
| 0x02000000 | AREA_FLAG_INSIDE         | 用于在 Map::IsOutdoors 中判断与法术相关的室内/室外问题                                                                                                               |
| 0x04000000 | AREA_FLAG_OUTSIDE        | 用于在 Map::IsOutdoors 中判断与法术相关的室内/室外问题                                                                                                               |
| 0x08000000 | AREA_FLAG_WINTERGRASP_2  | 冬拥湖及其子区域                                                                                                                                                                  |
| 0x20000000 | AREA_FLAG_CANNOT_FLY     | 不允许飞行，仅在达拉然区域（区域 4395）使用                                                                                                                                     |
