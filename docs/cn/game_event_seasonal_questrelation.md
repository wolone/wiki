...
**\`game\_event\_seasonal\_questrelation\` 表**

保存关于游戏事件季节性任务关系的信息，以允许重置 ZoneOrSort 为 -22 的任务。

**表结构**

| 字段 (Field)     | 类型 (Type) | 属性 (Attributes) | 键 (Key) | 空 (Null) | 默认 (Default) | 额外 (Extra) | 注释 (Comment)       |
| ---------------- | ----------- | ----------------- | -------- | --------- | -------------- | ------------ | -------------------- |
| [questId][1]     | INT         | UNSIGNED          | PRI      | NO        | 0              |              | 任务标识符           |
| [eventEntry][2]  | MEDIUMINT   | UNSIGNED          | PRI      | NO        | 0              |              | 游戏事件的条目       |
...
<summary>点击此处查看表的内容（2024/09/29）</summary>
