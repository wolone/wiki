...
**\`vehicle\_template\_accessory\` 表**

此表中的记录可被 [vehicle\_accessory](vehicle_accessory) 表覆盖。
...
用作载具的生物的 entry。取自 [creature_template](creature_template#entry)。

### accessory\_entry

用作主载具的骑乘者/炮塔/附加组件的 [creature_template](creature_template#entry) entry。取自 creature_template 的 ID。
