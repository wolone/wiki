...
该 DBC 文件包含所有独立阵营的信息。该 DBC 中的每个阵营条目都包含计算与该 DBC 中任何其他阵营之间敌视、友好或中立关系所需的全部数据。

**重要提示：这些值仅用于 [creature\_template](creature_template) 和 [gameobject\_template](gameobject_template) 表。**
...
| -------- | -------------------------------------------- | ---------------- |
| 1        | ID                                           | Int              |
| 2        | Name（引用 [Faction.dbc](faction)）          | Int              |
| 4        | ourMask                                      | Bitmask（4 位）  |
| 5        | friendlyMask                                 | Bitmask（4 位）  |
| 6        | hostileMask                                  | Bitmask（4 位）  |
| 7-10     | enemyFactions                                | Int              |
| 11-14    | friendFactions                               | Int              |

# **说明与计算**

每个不同的条目与其他任何阵营都有不同的关系。

计算敌视或友好关系最简单的方法是查看下表列中的 Friend factions（友方阵营）和 Enemy factions（敌方阵营）列。如果我们的阵营在敌方阵营列表中有其他阵营的 ID，那么它（至少）对该阵营是敌对的。友方阵营列的友好关系检查同理。

如果我们的阵营在敌方或友方阵营列表中没有其他阵营的 ID，那么我们在计算敌视或友好关系时就需要更加复杂。
...
第一个掩码（ourMask）定义了我们正在处理什么类型的阵营。例如，阵营 1（PLAYER, Human）的 ourMask 为 3；这意味着它设置了前两个位，因此它既是"所有玩家"也是"联盟玩家"。

第二个掩码（friendlyMask）定义了该阵营与哪些阵营组友好。这意味着如果该字段包含某个阵营组的位，那么它对该阵营就是友好的。例如，阵营 5（PLAYER, Undead）的 friendlyMask 为 4；这意味着它设置了第三个位，因此它与"部落玩家"友好。

第三个掩码（hostileMask）定义了该阵营与哪些阵营组敌对。该阵营将对任何在该字段中设置了位的阵营组保持敌对。例如，让我们检查前两个暴风城阵营。第一个（阵营 11）的 hostileMask 为 12。其二进制表示为 1100，设置了第三和第四位。第二个暴风城阵营（阵营 12）的 hostileMask 为 4。其二进制表示为 0100，设置了第三位。将公式应用于这两个值，我们可以看到阵营 11 对部落玩家以及任何非玩家的东西都保持敌对，而阵营 12 只对部落玩家敌对。阵营 12 不会攻击任何属于怪物阵营组的生物。

对于喜欢数学的读者，测试友好关系的基本公式是

``` cpp
(friendlyMask & other.ourMask) != 0 
```

...其中 other 是另一个阵营。敌视关系同理：

``` cpp
(hostileMask & other.ourMask) != 0
```

对于不那么喜欢数学的读者，我们将不得不比较两个二进制数。
首先，您需要将所有掩码转换为四位的二进制形式；因此 1 变成 0001，2 变成 0010，3 变成 0011，以此类推。

其次，将 friendlyMask 或 hostileMask 的值与 ourMask 的值对齐。例如：

``` cpp
     1100 //阵营 11 的 hostileMask
     1000 //阵营 14 的 ourMask
```

接下来，对这两个数字执行逻辑'AND'运算，将下面的数字与上面的数字对齐，并遵循以下规则：
...
最后，检查你得到的结果与值 0 的比较。只有 0000 等于 0，其他任何值都不等于 0。如果结果**'不'**等于 0，那么该阵营与另一个阵营是敌对/友好的。在我们的例子中，很明显结果不是 0（如果再次转换为十进制，实际上是 8），所以阵营 11 对阵营 14 是敌对的，并会攻击它。

从以上所有计算中，我们注意到如果某个阵营的 ourMask 为 0（即 0000），它对一切都保持中立（不一定是友好）。如果某个阵营的 friendlyMask 为 0，那么它永远不会对任何事物或任何人友好（除非它在下表的 Friend factions 列中有其他阵营 ID）。最后，如果某个阵营的 hostileMask 为 0，它永远不会对任何事物或任何人敌对（同样，在下表的 Enemy factions 列中可能有例外）。

总结一下，您应该按照以下步骤计算阵营 A 是否对阵营 B 敌对：

1.  在下表中查找阵营 A 的 Enemy faction（敌方阵营）列表中的阵营 B
    -   如果找到，则阵营 A 对阵营 B 敌对，您可以结束
    -   如果未找到，则继续执行下一步
2.  在下表中查找阵营 A 的 hostileMask
3.  在下表中查找阵营 B 的 ourMask
4.  对这两个值执行 and (&) 运算
    -   如果结果为 0，则阵营 A 对阵营 B 不敌对（但也未必友好）
    -   如果结果不为 0，则阵营 A 对阵营 B **是**敌对的

以及这些步骤来计算阵营 A 是否对阵营 B 友好：

1.  在下表中查找阵营 A 的 Friend faction（友方阵营）列表中的阵营 B
    -   如果找到，则阵营 A 对阵营 B 友好，您可以结束
    -   如果未找到，则继续执行下一步
2.  在下表中查找阵营 A 的 friendlyMask
3.  在下表中查找阵营 B 的 ourMask
4.  对这两个值执行 and (&) 运算
    -   如果结果为 0，则阵营 A 对阵营 B 不友好（但也未必敌对）
    -   如果结果不为 0，则阵营 A 对阵营 B **是**友好的

# **内容**

**版本为：3.3.5a**

| ID   | 名称                                | ourMask | friendlyMask | hostileMask | 敌方阵营          | 友方阵营       |
| ---- | ----------------------------------- | ------- | ------------ | ----------- | ----------------- | --------------- |
